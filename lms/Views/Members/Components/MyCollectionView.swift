...
struct LibraryScreen: View {
    ...
    private let tabs = ["History", "Issued", "Requested", "Wishlist"]
    
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                HStack {
                    // Title at the left side
                    Text("My Collection")
                        .font(.largeTitle)
                        .fontWeight(.bold)
                        .frame(maxWidth: .infinity, alignment: .leading)
                    
                    // View toggle buttons at the right side
                    HStack(spacing: 4) {
...
//            loadMockData()
    ...
    private func bookCollectionView(for tabIndex: Int) -> some View {
        VStack {
            ...
            if tabIndex == 0 { // For logs tab
                VStack {
                    if bookService.isLoading {
                        ProgressView("Loading history...")
                            .frame(maxWidth: .infinity, maxHeight: .infinity)
                    } else if filteredBooks(for: tabIndex).isEmpty {
                        VStack(spacing: 16) {
                            ...
                            Text("Books you've borrowed and returned will appear here")
                                .font(.subheadline)
            ...
            else if tabIndex == 3 { // For wishlist tab
                VStack {
                    if bookService.isLoading {
            ...
            else { // For Borrowed (tab 1) and Reserved (tab 2)
                if bookService.isLoading {
                    ProgressView(tabIndex == 1 ? "Loading issued books..." : "Loading requested books...")
                        .frame(maxWidth: .infinity, maxHeight: .infinity)
                } else if filteredBooks(for: tabIndex).isEmpty {
                    VStack(spacing: 16) {
                        // Different icons for Issued vs Requested tabs
                        Image(systemName: tabIndex == 1 ? "book.closed" : "clock")
                            .font(.system(size: 60))
                            .foregroundColor(.gray)
                        
                        Text(tabIndex == 1 ? "No issued books" : "No requested books")
                            .font(.headline)
                        
                        Text(tabIndex == 1 ?
                             "Books you currently have borrowed will appear here" :
                                "Books you've requested will appear here")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                        .padding(.horizontal)
    ...
    private func filteredBooks(for tabIndex: Int) -> [BookWithStatus] {
        ...
        case 0: // My Collection - all books
            tabBooks = bookService.historyBooks
        case 1: // Borrowed
            // Convert currently reading books to BookWithStatus format
            return bookService.currentlyReadingBooks.map { book in
                BookWithStatus(
                    book: book,
                    status: .borrowed,
                    borrowedDate: Date(), // You may want to fetch actual dates if available
                    dueDate: nil,
                    returnedDate: nil
                )
            }
        case 2: // Reserved
            // Convert reserved books to BookWithStatus format
            return bookService.reservedBooks.map { book in
                BookWithStatus(
                    book: book,
                    status: .reserved,
                    borrowedDate: Date(), // Use actual reservation date if available
                    dueDate: nil,
                    returnedDate: nil
                )
            }
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            ZStack {
                ...
                if let imageURL = book.book.imageURL {
                    ...
                    // Fallback for when there's no image URL
                    Image(systemName: "book.closed")
                        .font(.system(size: 30))
                        .foregroundColor(.white.opacity(0.7))
                }
